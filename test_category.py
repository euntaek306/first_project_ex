...
def get_chatbot_response(user_message, context=""):
    if not client:
        return "⚠️ OpenAI API 클라이언트가 초기화되지 않았습니다. .env 파일의 OPENAI_API_KEY를 확인해주세요."
        ...
        if st.session_state.chat_history:
            ...
            for idx, msg in enumerate(st.session_state.chat_history):
                if msg["role"] == "user":
                    ...
                    if st.button(f"💬 {summary}", key=f"history_{idx}", use_container_width=True):
                        # 해당 대화로 스크롤하기 위한 플래그 설정
                        st.session_state.scroll_to_index = idx
                        st.rerun()
        else:
            st.info("아직 대화 기록이 없습니다.")
    
    # 메인 챗봇 화면 - 제목과 HOME 버튼
    col1, col2 = st.columns([9,2])
    with col1:
        st.markdown("<h1 style='text-align: center;'>🏃 러닝 가이드 챗봇</h1>", unsafe_allow_html=True)
    
    with col2:
        if st.button("🏠 HOME", key="chat_home_btn", type="secondary"):
            st.session_state.chat_mode = False
            st.session_state.chat_history = []
            st.session_state.initial_question = ""
            st.rerun()
